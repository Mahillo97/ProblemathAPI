...
def saveStatementDB(con, absoluteURL, tags, mag, prop):
	...
	try:
		...
		tupleValuesStatement = tupleValuesStatement + (texStatement,'placeholder','placeholder',0)
...
def saveSolutionDB(con, absoluteURLSolution, idProblem, solver):
	...
	try:
		...
		texSolu = texSolu[texSolu.find('\\begin{document}') + len('\\begin{document}'):texSolu.find('\\end{document}')]
