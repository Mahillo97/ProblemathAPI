...
def getPackagesProblem(con, problem_id):
    ...
    try:
        ...
        for package in packages_data:
            ...
            if(package[1]):
                packages.append((package[0].decode("utf-8"), package[1].decode("utf-8")))
            else:
...
def getProblemSheet(con, dictionaryProblems):
    ...
    try:
        ...
        problems_id = []
        keysP = list(filter(lambda x: x.startswith('problem'), dictionaryProblems.keys()))
        ...
        for package in packages:
            if type(package)==tuple:
                newTexFile.write('\\usepackage[' + package[1] + ']{' + package[0] + '}\n')
            else:
                newTexFile.write('\\usepackage{' + package + '}\n')
        ...
        for n in range(1, numProblems+1):
            ...
            for solution_id in solutionIdList:
                ...
                if(auxTex):
                    newTexFile.write('\\textbf{Solution ' + str(solutionsCounter) + '}\\\\\n')				
                    newTexFile.write(auxTex + '\n')
        ...
        cliCompile = 'pdflatex -jobname=' + urlNewTex.rsplit('.', 1)[0] + ' '+ urlNewTex
        os.system(cliCompile)
